...
param(
    [switch] $Ui,
    [string] $AssemblyPath = 'DirectPort.PowerShell.dll',
    [string] $AtlasPng = 'N:\bin\msdf-atlas-gen\cascadia-code-atlas.png',
    [string] $MetricsJson = 'N:\bin\msdf-atlas-gen\cascadia-code-metrics.json'
)

# PSX evaluates the file as a script block, so the usual script-file automatic
# variables can be empty. Recover the source directory from the host command
# line when possible, then fall back to the host's working directory.
$sourceRoot = $PSScriptRoot
if ([string]::IsNullOrWhiteSpace($sourceRoot)) {
    foreach ($argument in [Environment]::GetCommandLineArgs()) {
        if ([IO.Path]::GetExtension($argument) -ieq '.psx' -and [IO.File]::Exists($argument)) {
            $sourceRoot = [IO.Path]::GetDirectoryName([IO.Path]::GetFullPath($argument))
            break
        }
    }
}
...
if (-not [IO.Path]::IsPathFullyQualified($AssemblyPath)) {
    $AssemblyPath = [IO.Path]::GetFullPath([IO.Path]::Combine($sourceRoot, $AssemblyPath))
}
...
if (-not $Ui) {
    [string] $scriptSource = $MyInvocation.MyCommand.ScriptBlock
    if ([string]::IsNullOrWhiteSpace($scriptSource)) {
        throw 'PSX did not expose the script source needed to start the STA UI runspace.'
    }
    ...
    $ps.Runspace = $rs
    [void]$ps.AddScript($scriptSource).AddParameter('Ui').AddParameter('AssemblyPath', $AssemblyPath).AddParameter('AtlasPng', $AtlasPng).AddParameter('MetricsJson', $MetricsJson)
