...
if ($null -eq $script:QnnPreview) {
    $script:QnnPreview = [pscustomobject]@{
        Receipt = $null
        Error   = $null
    }
}
...
$global:labels = @(
    @{ Text = ' 1 QNN '; Mode = [CanvasMode]::Touch },
...
function global:Set-Mode([CanvasMode] $Mode) {
    ...
    if ($wasAnimated -and -not $isAnimated) {
        $script:WasAnimating = $true
    }
...
function global:Invoke-QnnPreviewProof {
    $script:QnnPreview.Receipt = $null
    $script:QnnPreview.Error = $null

    try {
        $script:QnnPreview.Receipt = Invoke-QnnMatMulAddProof
    }
    catch {
        $script:QnnPreview.Error = $_.Exception.Message

        [Android.Util.Log]::Error(
            'PowerShell',
            "QNN preview failed: $_ stack=$($_.ScriptStackTrace)")
    }
...
function global:Build-Cells([int] $ViewWidth, [int] $ViewHeight) {
    ...
    if ($mode -eq [CanvasMode]::Touch) {
        Put-Text $cells $columns $rows 3 3 ' ANDROIDSMA / QUALCOMM HEXAGON HTP ' 15 4
        Put-Text $cells $columns $rows 3 4 ' PowerShell -> QNN -> CDSP -> V73 HTP ' 7 0

        Put-Text $cells $columns $rows 3 6 ' [ RUN QNN PROOF ] ' 15 4

        if ($null -ne $script:QnnPreview.Error) {
            Put-Text $cells $columns $rows 3 8 ' STATUS: FAIL ' 15 1
            Put-Text $cells $columns $rows 3 10 (" ERROR: $($script:QnnPreview.Error) ") 9 0
        }
        elseif ($null -ne $script:QnnPreview.Receipt) {
            $r = $script:QnnPreview.Receipt
            $status = if ($r.Passed) { 'PASS' } else { 'FAIL' }

            Put-Text $cells $columns $rows 3 8 `
                (" STATUS: {0} " -f $status) `
                $(if ($r.Passed) { 15 } else { 9 }) `
                $(if ($r.Passed) { 2 } else { 1 })

            Put-Text $cells $columns $rows 3 10 `
                (" BACKEND: {0} " -f $r.BackendLibrary) 15 0

            Put-Text $cells $columns $rows 3 11 `
                (" GRAPH: {0} " -f ([string]::Join(' -> ', $r.GraphOperations))) 15 0

            Put-Text $cells $columns $rows 3 12 `
                (" SHAPE: [1,{0}] x [{0},{1}] + [1,{1}] " -f $r.FeatureK, $r.FeatureN) 7 0

            Put-Text $cells $columns $rows 3 14 `
                (" MAX REL: {0:0.00000} " -f [double]$r.MaximumRelativeError) 10 0

            Put-Text $cells $columns $rows 3 15 `
                (" ELAPSED: {0:0.000} ms  WHOLE-METHOD " -f [double]$r.ElapsedMilliseconds) 10 0

            Put-Text $cells $columns $rows 3 17 `
                (" PID: {0}  PS TID: {1}  RUNSPACE: {2} " -f
                    $r.ApplicationPid,
                    $r.PowerShellTid,
                    $r.RunspaceManagedIdentity) 8 0
        }
        else {
            Put-Text $cells $columns $rows 3 8 ' STATUS: READY ' 10 0
            Put-Text $cells $columns $rows 3 10 ' MatMul -> ElementWiseAdd ' 7 0
            Put-Text $cells $columns $rows 3 11 ' [1,32] x [32,32] + [1,32] ' 7 0
        }
    }
...
function global:Invoke-AndroidCanvasInput {
    ...
    elseif ($leftDown -and
            -not $script:AndroidCanvasEdge.PreviousLeft -and
            $script:Mode -eq [CanvasMode]::Touch -and
            $cellY -eq 6 -and
            $cellX -ge 3 -and
            $cellX -lt 21) {

        Invoke-QnnPreviewProof
    }
