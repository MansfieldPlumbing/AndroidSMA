...
using System.Text;
namespace Subsystem.Capabilities.Shim;
...
internal static class Shim
{
    public static int Run(string[] args)
    {
        ...
        // friction F8). ConsoleTerminal.SetShouldExit also captures `exit N`, so ss returns the script's code.
        var terminal = new ConsoleTerminal();
        var rs = RunspaceFactory.CreateRunspace(terminal, iss);
        rs.Open();
        ...
        try
        {
            ...
            foreach (var e in ps.Streams.Error)
            {
                ...
                {
                    string? closest = FindClosestCommand(cnfe.CommandName, rs);
                    if (closest != null)
                    {
    ...
    private static string? FindClosestCommand(string target, Runspace rs)
    {
        var candidates = new List<string>
        {
            "selftest", "diag", "contextualize", "onboard", "build", "check", "extract",
            "chat", "tts", "view", "ui", "mcp", "git", "status", "refs", "adb", "tui", "cell", "repl", "gateway"
        };
        
        try
        {
            var commands = rs.SessionStateProxy.InvokeCommand.GetCommands("*", CommandTypes.Cmdlet, true);
            foreach (var cmd in commands) candidates.Add(cmd.Name);
        }
        catch {}
        ...
        {
            int dist = LevenshteinDistance(target, c);
