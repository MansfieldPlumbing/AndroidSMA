using System;
using Subsystem.Vom;
namespace Subsystem.Capabilities.Surface;
...
public static class Notifications
{
    ...
    private const string NotificationChannelId = "notification_v2";
    private static bool _channelReady;
    private static void EnsureNotificationChannel(Android.Content.Context ctx, Android.App.NotificationManager nm) {
        if (_channelReady) return;
        var ch = new Android.App.NotificationChannel(NotificationChannelId, "Notification", Android.App.NotificationImportance.Default);
        try {
        ...
        try { nm.DeleteNotificationChannel("notification_v1"); }
        catch (Exception ex) { Subsystem.Dg.Log("notify", "notification_v1 retire skipped: " + ex.Message); }
        _channelReady = true;
    }
    ...
    public static void SendNotification(string title, string text) {
        try {
            ...
            if (nm != null) {
                ...
                var builder = new Android.App.Notification.Builder(ctx, NotificationChannelId)
