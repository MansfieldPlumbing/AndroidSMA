...
namespace Subsystem.Capabilities.LiteRt
{
    // In-process LiteRT-LM Runtime for the Windows head — CPU rung, P/Invoke against the litert-lm C API.
    public sealed class LiteRtRuntime : IGuestEngine, IDisposable
    {
        ...
        public void ProjectTurn(string prompt, byte[]? audioBytes, byte[]? imageBytes, Action<TurnEvent> callback, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                if (res != 0)
                {
                    callback(new TurnEvent(TurnEventKind.Error, $"Native send_message_stream failed: {res}", Fault: new TurnFault(TurnFaultClass.DecodeFaulted, _unitId, _backendName, $"Native send_message_stream failed: {res}")));
                }
            }
            finally
            {
