...
using Subsystem.Cm;
using Subsystem.Vom;
namespace Subsystem.Capabilities.Camera;
...
public static class Camera
{
    ...
    public static int Run(string[] args)
    {
        ...
        // The gate + bring-up live in VirtualCameraDevice (shared with the gateway tray); DrivePump just drives the pump.
        return DrivePump(name, grid, fps, grant);
    }

    // ss camera's PUMP: drive the shared VirtualCameraDevice from a console while-loop (the gateway tray drives the
    // SAME device from a WinForms timer — one device, two pumps; the pump is the only thing that changes).
    private static int DrivePump(string name, bool grid, int fps, bool grant)
    {
        var device = VirtualCameraDevice.Start(name, grid, fps, grant, out int code, out string? reason);
        if (device == null)
        {
            if (reason != null) Console.Error.WriteLine("ss camera: " + reason);
            if (code == 1)
                Console.Error.WriteLine("  the device artifact lives at src/native/virtuacam/DirectPortBroker.dll and is copied beside ss.exe at build.");
            return code;
        }

        Console.WriteLine($"ss camera: hosting device '{name}'  (mode={(grid ? "auto-discovery grid" : "priority-list")}, {fps} fps)");
        Console.WriteLine($"  VOM:      \\VirtualCamera\\{name}   (gated by {device.CapabilityPath})");
        ...
        while (!stop.IsSet)
        {
            device.RenderFrame();                           // discover -> composite -> signal the output fence
            var state = device.State;
            if (state != last) { Console.WriteLine($"  device state: {state}"); last = state; }
            stop.Wait(device.PeriodMs);                     // wakes immediately on Ctrl+C
        }

        Console.WriteLine($"\nss camera: stopping — Terminate(\\VirtualCamera) reclaims the device (ShutdownBroker)");
        device.Stop();                                      // cascades Reclaim -> ShutdownBroker deterministically
        return 0;
    }
