...
// VirtualCameraDevice — the in-proc VirtuaCam device as ONE concept (#9): gate -> probe -> InitializeBroker ->
...
// RenderFrame() from a console while-loop; the gateway tray drives the SAME device from a WinForms timer on
// its STA thread. One device, two pumps — the boundary is the only thing that changes.
internal sealed class VirtualCameraDevice
{
    ...
    private VirtualCameraDevice(Owner owner, string name, string capPath, int periodMs)
    {
    ...
    // device is initialized and registered — drive RenderFrame() on whatever single thread owns the pump.
    public static VirtualCameraDevice? Start(string name, bool grid, int fps, bool grant, out int code, out string? reason)
    {
        code = 0; reason = null;
        ...
        // Fail fast with a clear reason if the device DLL / its dependencies aren't present.
        ...
        return new VirtualCameraDevice(owner, name, capPath, periodMs);
    }

    // One render tick: discover -> composite -> signal the output fence. MUST be called on a single thread
    // (the device's D3D context is single-threaded) — never two pumps at once.
