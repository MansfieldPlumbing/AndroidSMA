...
using Subsystem;
namespace Subsystem.Capabilities.Chat
{
    internal static class Chat
    {
        public static int Run(string[] args)
        {
            ...
            try
            {
                ...
                StreamOutput(client, prompt, cts.Token);
        ...
        private static void StreamOutput(LiteRtRuntime client, string prompt, CancellationToken ct)
        {
            try
            {
                client.ProjectTurn(prompt, null, (delta) =>
                {
                    if (delta.Kind == TurnEventKind.Token && !string.IsNullOrEmpty(delta.Text))
                    {
                        Console.Write(delta.Text);
                    }
                    else if (delta.Kind == TurnEventKind.Think && !string.IsNullOrEmpty(delta.Text))
                    {
                        Console.ForegroundColor = ConsoleColor.Cyan;
                        Console.Write(delta.Text);
                        Console.ResetColor();
                    }
                    else if (delta.Kind == TurnEventKind.Error)
                    {
