...
public static class LiteRtGuest
{
    ...
    internal static string ResolveSystemPrompt(Context ctx)
    {
        try
        {
    ...
    private static bool TryAcquire(Context ctx, out IGuestEngine runtime)
    {
        runtime = null!;
        try
        {
            ...
            if (Subsystem.Vom.Vom.TryGetByPath(EngineOwner, EnginePath(spec.Id), out var h) &&
                GCHandle.FromIntPtr(h.Resource).Target is IGuestEngine r)
            {
                runtime = r;
                return true;
            }
    ...
    // verification when absent. Throws TurnFaultException (typed) on bring-up failure after demoting the
    // unit's registry record (§5).
    public static async Task<IGuestEngine> GetAsync(Context ctx, Func<string, Task>? report = null, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            if (Subsystem.Vom.Vom.TryGetByPath(owner, path, out var h) &&
                GCHandle.FromIntPtr(h.Resource).Target is IGuestEngine again)
            {
            ...
            IGuestEngine runtime = new LiteRtRuntime(file, spec.Id, ResolveSystemPrompt(ctx), ctx.CacheDir?.AbsolutePath, sampler: spec.Sampler);
            ...
            if (fault != null)
            {
                ...
                throw new TurnFaultException(fault);
            }
