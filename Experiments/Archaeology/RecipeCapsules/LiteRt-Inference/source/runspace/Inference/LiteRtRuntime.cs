...
namespace Subsystem
{
    ...
    // budget) in BringUp.
    public sealed class LiteRtRuntime : IGuestEngine
    {
        ...
        private TurnFault? _initFault;                // §3.1: typed bring-up fault; null = serviceable
        ...
        // The active turn's callback + splitter — the native callback forwards into these under _turnGate.
        private Action<TurnEvent>? _activeCallback;
        ...
        public TurnFault? BringUp()
        {
            ...
            lock (_gate)
            {
                ...
                _initFault = new TurnFault(TurnFaultClass.BringUpFailed, _unitId,
                    string.Join("/", admitted), lastDetail ?? "no admitted backend initialized");
                return _initFault;
            }
        }

        // Project one turn as structured events synchronously. Visible text and the thinking channel are split out of
        // the model's token stream by the splitter; the native callback drives both.
        public void ProjectTurn(string prompt, byte[]? audioBytes, Action<TurnEvent> callback, CancellationToken ct = default)
            => ProjectTurn(prompt, audioBytes, null, callback, ct);
        ...
        // untouched.
        public void ProjectTurn(string prompt, byte[]? audioBytes, byte[]? imageBytes, Action<TurnEvent> callback, CancellationToken ct = default)
        {
            var fault = BringUp();
            if (fault != null)
            {
                callback(new TurnEvent(TurnEventKind.Error, fault.NativeDetail, Fault: fault));
                return;
            }
            StreamOn(_conversationHandle!, prompt, audioBytes, imageBytes, callback, ct);
        }
        ...
        internal void StreamOn(LiteRtLmConversationHandle conv, string prompt, byte[]? audioBytes, byte[]? imageBytes, Action<TurnEvent> callback, CancellationToken ct = default)
        {
            var splitter = new ThinkingSplitter(callback);

            _turnGate.Wait(ct);

            // Pin THIS runtime so the native callback can resolve it from callback_data without a delegate
            // allocation. Freed in the finally, after the synchronous send returns.
            ...
            try
            {
                ...
                if (res != 0)
                {
                    callback(new TurnEvent(TurnEventKind.Error, $"send_message_stream returned {res}", Fault: new TurnFault(TurnFaultClass.DecodeFaulted, _unitId, _backendName, $"send_message_stream returned {res}")));
                }
        ...
        public SecondaryTurn? OpenSideConversation(string? systemPrompt, ToolDescriptor[]? tools)
        {
        ...
        internal void OnChunk(string chunk, bool isFinal, string errorMsg)
        {
            try
            {
                ...
                {
                    var detail = errorMsg;
                    var cls = detail.Contains("not alive", StringComparison.OrdinalIgnoreCase) ? TurnFaultClass.ConversationDefunct
                            : detail.Contains("cancel", StringComparison.OrdinalIgnoreCase)    ? TurnFaultClass.DecodeCancelled
                            : TurnFaultClass.DecodeFaulted;

                    _activeCallback?.Invoke(new TurnEvent(TurnEventKind.Error, detail, Fault: new TurnFault(cls, _unitId, _backendName, detail)));
                    return;
                }
            ...
            {
                _activeCallback?.Invoke(new TurnEvent(TurnEventKind.Error, ex.Message, Fault: new TurnFault(TurnFaultClass.DecodeFaulted, _unitId, _backendName, ex.Message)));
            }
        }
        ...
        // core (StreamOn); Dispose frees only this conversation's KV — the resident one is untouched.
        private sealed class LiteRtSideConversation : SecondaryTurn
        {
            ...
            public override void ProjectTurn(string prompt, Action<TurnEvent> callback, CancellationToken ct = default)
            {
                var c = _conv ?? throw new ObjectDisposedException(nameof(LiteRtSideConversation));
                _runtime.StreamOn(c, prompt, null, null, callback, ct);
            }
        ...
        private sealed class ThinkingSplitter
        {
            ...
            private readonly Action<TurnEvent> _callback;
            private int _emittedThink, _emittedAnswer;
            private string _buf = "";

            public ThinkingSplitter(Action<TurnEvent> callback) { _callback = callback; }
            ...
            public void Push(string chunk)
            {
                ...
                if (think.Length > _emittedThink) { _callback(new TurnEvent(TurnEventKind.Think, think.Substring(_emittedThink))); _emittedThink = think.Length; }
                if (answer.Length > _emittedAnswer) { _callback(new TurnEvent(TurnEventKind.Token, answer.Substring(_emittedAnswer))); _emittedAnswer = answer.Length; }
            }
